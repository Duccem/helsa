"use client";
import { type ColumnDef, flexRender, type Table as ReactTable, type Row } from "@tanstack/react-table";
import {
  ArrowDown,
  ArrowUp,
  ChevronDown,
...
export function SortableHeader<T>({
  ...
  if (isVisible(name)) {
    return (
      <div
        className=" cursor-pointer justify-start hover:bg-primary/30 rounded-md inline-flex items-center gap-1 p-2"
...
export function ServerPagination({ meta }: { meta: { page: number; pages: number; pageSize: number; total: number } }) {
  const searchParams = useSearchParams();
  const pathname = usePathname();
  const router = useRouter();
    ...
    <Table>
      <TableHeader>
        {table.getHeaderGroups().map((headerGroup) => (
          <TableRow key={headerGroup.id} className="h-[40px] md:h-[45px] select-text  hover:bg-transparent">
            {headerGroup.headers
              .filter((header) => header.column.getIsVisible())
              .map((header) => {
                return (
                  <TableHead key={header.id} className="px-3 md:px-4 py-2 ">
      ...
      <TableBody>
        {table.getRowModel().rows.length ? (
          table.getRowModel().rows.map((row) => (
            <TableRow
              key={row.id}
              className={cn("h-[40px] md:h-[45px] select-text  hover:bg-transparent", {
                "cursor-pointer": setOpenAction,
              })}
            >
              {row.getVisibleCells().map((cell) => (
                <TableCell
                  ...
                  onClick={() => {
                    if (cell.column.id !== "select" && cell.column.id !== "actions" && setOpenAction) {
                      setOpenAction(row.id);
                    }
                  }}
                >
...
export function ServerTableLoading({ cellNumber }: { cellNumber: number }) {
  ...
  return (
    <div className="w-full animate-pulse mb-8 relative border rounded-md">
      <Table>
        <TableHeader>
          <TableRow className="h-[40px] md:h-[45px] select-text  hover:bg-transparent">
            {cells.map((cell) => (
...
export function TableVisibility<T>({ table }: { table: ReactTable<T> }) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        render={
