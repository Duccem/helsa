  ...
  const { data, isFetching, fetchNextPage, hasNextPage } = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => {
      ...
      const ordersWithMetadata = {
        ...
        meta: {
          hasNextPage: (pageParam ? Number(pageParam) : 1) < (pagination?.maxPage ?? 10),
          cursor:
  ...
  return (
    <div
      className={cn(
        ...
        status === "pending" && "bg-[#FFD02B]/10 text-[#FFD02B] dark:bg-[#FFD02B]/10 dark:text-[#FFD02B]",
        (status === "cancelled" || status === "canceled") &&
          "text-[#878787] bg-[#F2F1EF] text-[10px] dark:text-[#878787] dark:bg-[#1D1D1D]",
        status === "failed" && "text-[#1D1D1D] bg-[#878787]/10 dark:text-[#F5F5F3] dark:bg-[#F5F5F3]/10",
        className,
      )}
    >
